...
+ (void)initialize
{
    ...
    // 2.创表
    [_db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_status (id integer PRIMARY KEY, status blob NOT NULL, rawid integer NOT NULL, content text NOT NULL, user_name text NOT NULL);"];
}
...
{
    if (!text) return nil;
    NSString *sql = [NSString stringWithFormat:@"SELECT * FROM t_status WHERE content LIKE '%%%@%%' OR user_name LIKE '%%%@%%' ORDER BY rawid DESC;",text,text];
    FMResultSet *set = [_db executeQuery:sql];
    NSMutableArray *statuses = [NSMutableArray array];
    while (set.next) {
        NSData *statusData = [set objectForColumnName:@"status"];
